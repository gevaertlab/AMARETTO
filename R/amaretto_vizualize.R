  ...
  function(AMARETTOinit,
    ...
    if (ModuleNr > AMARETTOresults$NrModules) {
      stop(
        "\tCannot plot Module",
        ModuleNr,
        " since the total number of modules is",
        AMARETTOresults$N,
        ".\n"
      )
    }
    ModuleData <-
      as.data.frame(AMARETTOinit$MA_matrix_Var)[AMARETTOresults$ModuleMembership == ModuleNr,]
    ModuleRegulators <-
      AMARETTOresults$AllRegulators[which(AMARETTOresults$RegulatoryPrograms[ModuleNr,] != 0)]
    RegulatorData <-
      as.data.frame(AMARETTOinit$RegulatorData)[ModuleRegulators,]
    ModuleGenes <- rownames(ModuleData)
    cat(
      "Module",
      ModuleNr,
      "has",
      length(rownames(ModuleData)),
      "genes and",
      length(ModuleRegulators),
      "regulators for",
      length(colnames(ModuleData)),
      "samples.\n"
    )
    Alterations <-
      tibble::rownames_to_column(as.data.frame(AMARETTOinit$RegulatorAlterations$Summary),
                                 "HGNC_symbol") %>%
      dplyr::rename(DriverList = "Driver List") %>% dplyr::filter(HGNC_symbol %in% ModuleRegulators)
    Alterations <-
      Alterations %>% dplyr::mutate(
    ...
    ha_drivers <-
      HeatmapAnnotation(
        df = column_to_rownames(Alterations, "HGNC_symbol") %>% dplyr::select(CNVMet_Alterations,
                                                                              DriversList_Alterations),
        col = list(
          CNVMet_Alterations = c(
            `Copy number alterations` = "#eca400",
            `Methylation aberrations` = "#006992",
            `Methylation and copy number alterations` = "#d95d39",
            `Not Altered` = "lightgray"
          ),
          DriversList_Alterations = c(
            `Driver not predefined` = "lightgray",
            `Driver predefined` = "#588B5B"
          )
        ),
        which = "column",
        height = unit(0.3, "cm"),
    ...
    if (is.null(MET_matrix) && is.null(CNV_matrix)) {
      ...
      overlapping_samples <-
        Reduce(intersect, list(colnames(CNV_matrix), colnames(ModuleData)))
    } else if (is.null(CNV_matrix)) {
      overlapping_samples <-
        Reduce(intersect, list(colnames(MET_matrix), colnames(ModuleData)))
    } else {
      overlapping_samples <-
    ...
    if (is.null(order_samples)) {
      overlapping_samples_clust <-
        overlapping_samples[order(colMeans(ModuleData[, overlapping_samples]))]
    } else if (order_samples == "clust") {
      SampleClustering <-
        hclust(dist(t(ModuleData[, overlapping_samples])), method = "complete", members = NULL)
      overlapping_samples_clust <-
        overlapping_samples[SampleClustering$order]
    } else {
      print(
        "ordering type not recognized, samples will be orderd based on mean expression of the module genes"
      )
      overlapping_samples_clust <-
        overlapping_samples[order(colMeans(ModuleData[, overlapping_samples]))]
    }
    ...
    ClustRegulatorData <-
      t(RegulatorData[, overlapping_samples_clust])
    ...
    ha_Reg <-
      Heatmap(
        ...
        top_annotation = ha_drivers,
        column_title_gp = gpar(fontsize = 6, fontface = "bold"),
        col = colorRamp2(c(-max(
          abs(ClustRegulatorData)
        ),
        0, max(
          abs(ClustRegulatorData)
    ...
    ha_Module <-
      Heatmap(
        ...
        show_column_names = TRUE,
        show_row_names = FALSE,
        column_names_gp = gpar(fontsize = fontsizeMo),
    ...
    if (!is.null(MET_matrix)) {
      METreg <-
        intersect(rownames(AMARETTOinit$RegulatorAlterations$MET),
                  ModuleRegulators)
      print("MET regulators will be included when available")
      if (length(METreg) > 0) {
        MET_matrix = as.data.frame(MET_matrix)
        METData2 = METData = as.matrix(MET_matrix[unlist(Alterations %>% dplyr::filter(MET == 1) %>% dplyr::select(HGNC_symbol)),
                                                  overlapping_samples_clust])
        METData2[which(METData > 0)] <-
          "Hyper-methylated"  # hyper
        METData2[which(METData < 0)] <-
          "Hypo-methylated"  # hypo
        METData2[which(METData == 0)] <-
          "Not altered"  # nothing
        METData2 <- t(METData2)
        Metwidth = ncol(METData2) * 0.5
        Met_col = structure(
          c("#006992", "#d95d39", "white"),
          names = c("Hyper-methylated", "Hypo-methylated", "Not altered")
        )
        ha_Met <-
          Heatmap(
            METData2,
            name = "Methylation State",
            column_title = "Methylation State",
            cluster_rows = FALSE,
            cluster_columns = TRUE,
            show_column_dend = FALSE,
            show_column_names = TRUE,
    ...
    if (!is.null(CNV_matrix)) {
      CNVreg <-
        intersect(rownames(AMARETTOinit$RegulatorAlterations$CNV),
                  ModuleRegulators)
      print("CNV regulators will be included when available")
      if (length(CNVreg) > 0) {
        CNV_matrix = as.data.frame(CNV_matrix)
        CNVData2 = CNVData = as.matrix(CNV_matrix[unlist(Alterations %>% dplyr::filter(CNV == 1) %>% dplyr::select(HGNC_symbol)),
                                                  overlapping_samples_clust])
        CNVData2[which(CNVData >= 0.1)] <-
          "Amplified"  # amplification
        CNVData2[which(CNVData <= (-0.1))] <-
          "Deleted"  # deletion
        CNVData2[which(CNVData < 0.1 &
                         CNVData > (-0.1))] <- "Not altered"  # nothing
        CNVData2 <- t(CNVData2)
        CNVwidth = ncol(CNVData2) * 0.5
        CNV_col = structure(
          c("#006992", "#d95d39", "white"),
          names = c("Deleted", "Amplified", "Not altered")
        )
        ha_CNV <-
          Heatmap(
            CNVData2,
            name = "CNV State",
            column_title = "CNV State",
            cluster_rows = FALSE,
            cluster_columns = TRUE,
            show_column_dend = FALSE,
            show_column_names = TRUE,
    ...
    if (!is.null(SAMPLE_annotation)) {
      if (ID %in% colnames(SAMPLE_annotation)) {
        SAMPLE_annotation_fil <-
          as.data.frame(SAMPLE_annotation) %>% dplyr::filter(!!as.name(ID) %in% overlapping_samples_clust)
        suppressWarnings(
          SAMPLE_annotation_fil <-
            left_join(
              as.data.frame(overlapping_samples_clust),
              SAMPLE_annotation_fil,
              by = c(overlapping_samples_clust = ID)
            )
        )
        SAMPLE_annotation_fil <-
          column_to_rownames(SAMPLE_annotation_fil, "overlapping_samples_clust")
        cat(nrow(SAMPLE_annotation_fil),
            "samples have an annotation.\n")
