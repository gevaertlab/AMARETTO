...
CreateRegulatorData <-
  function(MA_matrix = MA_matrix,
    ...
    if (nrow(CNV_matrix) > 0) {
      CNV_matrix = FindTranscriptionallyPredictive_CNV(
        MA_matrix,
        CNV_matrix,
    ...
    if (nrow(MET_matrix) > 1) {
      METcounts = apply(MET_matrix, 1, function(x)
        length(unique(x)))
    ...
    MET_drivers <-
      intersect(rownames(MET_matrix), rownames(MA_matrix))
    ...
    if (!is.null(Driver_list)) {
      ...
      cat("\tFound",
          length(DriversList),
          "driver genes from the input list.\n")
    }
    if (is.null(Driver_list))
      Drivers <-
      unique(c(rownames(CNV_matrix), MET_drivers, DriversList))
    ...
    cat("\tFound a total of",
        length(Drivers),
        "unique drivers with your selected method.\n")
    if (length(Drivers) == 0) {
      ...
      MET_aberrations <-
        matrix(0, ncol = 3, nrow = length(MET_drivers))
      colnames(MET_aberrations) <-
        c("Hypo-methylated", "No_change", "Hyper-methylated")
      rownames(MET_aberrations) <- MET_drivers
      if (length(MET_drivers) > 0) {
        MET_aberrations[, 1] <-
          rowSums(MET_matrix[MET_drivers,] > 0) / ncol(MET_matrix)
        MET_aberrations[, 2] <-
          rowSums(MET_matrix[MET_drivers,] < 0) / ncol(MET_matrix)
        MET_aberrations[, 3] <-
          rowSums(MET_matrix[MET_drivers,] == 0) / ncol(MET_matrix)
      ...
      CNV_alterations <-
        matrix(0, ncol = 3, nrow = nrow(CNV_matrix))
      colnames(CNV_alterations) <-
        c("Amplification", "No_change", "Deletion")
      ...
      if (nrow(CNV_alterations) > 0) {
        ...
        CNV_alterations[, 2] <-
          rowSums(CNV_matrix < 0) / ncol(CNV_matrix)
        CNV_alterations[, 3] <-
          rowSums(CNV_matrix == 0) / ncol(CNV_matrix)
      }
      driverList_alterations <-
        matrix(1, ncol = 1, nrow = length(DriversList))
      ...
      Alterations[which(Drivers %in% rownames(CNV_matrix)), 1] <-
        rep(1, length(which(Drivers %in% rownames(CNV_matrix))))
      Alterations[which(Drivers %in% rownames(MET_matrix)), 2] <-
        rep(1, length(which(Drivers %in% rownames(MET_matrix))))
      Alterations[which(Drivers %in% rownames(driverList_alterations)), 3] <-
        rep(1, length(which(
          Drivers %in% rownames(driverList_alterations)
        )))
      Alterations <-
        list(
          MET = MET_aberrations,
...
FindTranscriptionallyPredictive_CNV <-
  function(MA_matrix,
    ...
    if (length(OverlapGenes) == 1) {
      CNV_TCGA_temp = MA_matrix_temp <-
        matrix(0, length(OverlapGenes), length(OverlapSamples))
      rownames(CNV_TCGA_temp) = rownames(MA_matrix_temp) <-
        OverlapGenes
      colnames(CNV_TCGA_temp) = colnames(MA_matrix_temp) <-
        OverlapSamples
      CNV_TCGA_temp[1,] <-
        CNV_matrix[OverlapGenes, OverlapSamples]
      MA_matrix_temp[1,] <-
        MA_matrix[OverlapGenes, OverlapSamples]
    ...
    if (length(OverlapGenes) > 0 && length(OverlapSamples) > 0) {
      ...
      for (i in 1:length(rownames(CNV_matrix))) {
        ...
        if (res$coefficients[2] > 0 &
            res.summary$coefficients[2, 4] < PvalueThreshold &
            res.summary$r.squared >
            RsquareThreshold) {
