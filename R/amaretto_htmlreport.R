...
#' @param phenotype_association_table Table with phenotypes and type of tests to perform.
...
AMARETTO_HTMLreport <- function(AMARETTOinit, AMARETTOresults, 
    ...
    output_address = "./", MSIGDB = FALSE, GMTURL = FALSE, phenotype_association_table=NULL) {
...
#    registerDoParallel(cluster, cores = NrCores)
    ...
    ModuleOverviewTable <- foreach::foreach(ModuleNr = 1:NrModules, 
      ...
      if (!is.null(phenotype_association_table)){
        module_phenotype_association_datatable<-DT::datatable(phenotype_association_table%>%filter(ModuleNr==paste0("Module ",ModuleNr))%>%
