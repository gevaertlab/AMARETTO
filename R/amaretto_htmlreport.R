...
AMARETTO_HTMLreport <-
  function(AMARETTOinit,
    ...
    dir.create(
      paste0(report_address, "htmls/modules"),
      recursive = TRUE,
      showWarnings = FALSE
    )
    ...
    if (hyper_geo_test_bool) {
      ...
      output_hgt <-
        HyperGTestGeneEnrichment(hyper_geo_reference,
                                 "./Modules_targets_only.gmt",
                                 NrCores)
      GeneSetDescriptions <-
        GeneSetDescription(hyper_geo_reference)
    }
    cat("The hyper geometric test results are calculated.\n")
    cluster <-
      makeCluster(c(rep("localhost", NrCores)), type = "SOCK")
    registerDoParallel(cluster, cores = NrCores)
    
    full_path <- normalizePath(report_address)
    ModuleOverviewTable <-
      foreach(
        ModuleNr = 1:NrModules,
        .packages = c("AMARETTO", "tidyverse", "DT", "rmarkdown")
      ) %dopar%
      {
        heatmap_module <-
        ...
        ModuleRegulators <-
          AMARETTOresults$RegulatoryPrograms[ModuleNr, which(AMARETTOresults$RegulatoryPrograms[ModuleNr,] != 0)]
        
        dt_regulators <-
          datatable(
            rownames_to_column(as.data.frame(ModuleRegulators), "RegulatorIDs") %>% dplyr::rename(Weights = "ModuleRegulators") %>%
              mutate(
                RegulatorIDs = paste0(
          ...
          formatRound("Weights", 3) %>% formatStyle("Weights", color = styleInterval(0, c("darkblue", "darkred")))
        
        if (hyper_geo_test_bool) {
          output_hgt_filter <-
            output_hgt %>% dplyr::filter(Testset == paste0("Module_", as.character(ModuleNr))) %>%
            dplyr::arrange(padj)
          output_hgt_filter <-
            left_join(output_hgt_filter,
                      GeneSetDescriptions,
                      by = c(Geneset = "GeneSet")) %>%
            mutate(overlap_perc = n_Overlapping / NumberGenes) %>% dplyr::select(
              Geneset,
              Description,
              n_Overlapping,
              Overlapping_genes,
          ...
          if (MSIGDB == TRUE & GMTURL == FALSE) {
            dt_genesets <-
              datatable(
                output_hgt_filter %>% mutate(
                  Geneset = paste0(
                    "<a href=\"http://software.broadinstitute.org/gsea/msigdb/cards/",
                    Geneset,
                    "\">",
                    gsub("_", " ", Geneset),
                    "</a>"
                ...
                escape = FALSE
              ) %>% formatSignif(c("p_value", "padj", "overlap_perc"), 2) %>% formatStyle(
                "overlap_perc",
            ...
            dt_genesets <-
              datatable(
                output_hgt_filter %>% dplyr::select(-Description) %>% mutate(
                  Geneset = paste0(
                    "<a href=\"http://software.broadinstitute.org/gsea/msigdb/cards/",
                    Geneset,
                    "\">",
            ...
            dt_genesets <-
              datatable(
                ...
                escape = FALSE
              ) %>% formatSignif(c("p_value", "padj", "overlap_perc"),
                                 2) %>% formatStyle(
                                   "overlap_perc",
                                   background = styleColorBar(c(0, 1), "lightblue"),
                                   backgroundSize = "98% 88%",
          ...
          ngenesets <-
            nrow(output_hgt_filter %>% dplyr::filter(padj < 0.05))
        } else {
          dt_genesets <-
            "Genesets were not analysed as they were not provided."
          ngenesets <- "NA"
        }
        modulemd <-
          paste0(full_path, "/htmls/modules/module", ModuleNr, ".rmd")
        file.copy(
          system.file("templates/TemplateReportModule.Rmd", package = "AMARETTO"),
          modulemd
        )
        rmarkdown::render(
          modulemd,
        ...
        return(c(
          ModuleNr,
          length(which(
            AMARETTOresults$ModuleMembership == ModuleNr
          )),
    ...
    colnames(ModuleOverviewTable) <-
      c("ModuleNr", "NrTarGenes", "NrRegGenes", "SignGS")
    ...
    all_targets <-
      rownames_to_column(data.frame(AMARETTOinit$ModuleMembership), "Genes") %>% dplyr::rename(Module = "AMARETTOinit.ModuleMembership") %>%
      mutate(Type = "Target")
    all_regulators <-
      melt(rownames_to_column(
        as.data.frame(AMARETTOresults$RegulatoryPrograms),
        "Module"
      ), id.vars = "Module") %>%
      dplyr::filter(value > 0) %>% dplyr::select(variable, Module) %>% mutate(Module = sub("Module_", "", Module),
                                                                              Type = "Regulator") %>% dplyr::rename(Genes = "variable")
    all_genes <-
      rbind(all_targets, all_regulators) %>% arrange(Genes) %>% dplyr::mutate(Module = paste0(
        "<a href=\"./modules/module",
        Module,
        ".html\">Module ",
        Module,
    ...
    if (hyper_geo_test_bool) {
      dt_genesetsall <-
        datatable(
          left_join(
            output_hgt %>% dplyr::filter(padj < 0.05 &
                                           n_Overlapping > 1) %>% dplyr::group_by(Geneset) %>%
              dplyr::mutate(
                Testset = paste0(
                  "<a href=\"./modules/module",
                  sub("Module_", "", Testset),
              ...
              ) %>% summarise(Modules = paste(Testset, collapse = ", ")),
            GeneSetDescriptions,
            by = c(Geneset = "GeneSet")
          ) %>%
            mutate(
    ...
    rmarkdown::render(
      system.file("templates/TemplateIndexPage.Rmd", package = "AMARETTO"),
...
HyperGTestGeneEnrichment <-
  function(gmtfile,
    ...
    test.gmt <-
      readGMT(testgmtfile)  # our gmt_file_output_from Amaretto
    ...
    resultloop <-
      foreach(j = 1:length(test.gmt), .combine = "rbind") %do% {
        # print(j)
        foreach(i = 1:length(gmt.path), .combine = "rbind") %dopar% {
          ...
          if (k > 0) {
            ...
            overlapping.genes <-
              paste(overlapping.genes, collapse = ", ")
            c(
              Geneset = names(gmt.path[i]),
    ...
    stopCluster(cluster)
    resultloop <-
      as.data.frame(resultloop, stringsAsFactors = FALSE)
    resultloop$p_value <- as.numeric(resultloop$p_value)
    resultloop$n_Overlapping <-
      as.numeric((resultloop$n_Overlapping))
    resultloop[, "padj"] <-
      p.adjust(resultloop[, "p_value"], method = "BH")
    return(resultloop)
  }
...
GmtFromModules <- function(AMARETTOinit, AMARETTOresults) {
  ModuleMembership <-
    rownames_to_column(as.data.frame(AMARETTOresults$ModuleMembership),
                       "GeneNames")
  NrModules <- AMARETTOresults$NrModules
  ModuleMembership <- ModuleMembership %>% arrange(GeneNames)
  
  ModuleMembers_list <-
    split(ModuleMembership$GeneNames, ModuleMembership$ModuleNr)
  names(ModuleMembers_list) <-
    paste0("Module_", names(ModuleMembers_list))
  
  gmt_file = "./Modules_targets_only.gmt"
  write.table(
    sapply(names(ModuleMembers_list), function(x)
      paste(x, paste(ModuleMembers_list[[x]], collapse = "\t"),
            sep = "\t")),
    gmt_file,
    quote = FALSE,
    row.names = TRUE,
...
GeneSetDescription <- function(filename) {
  ...
  colnames(gmtLines_description) <-
    c("GeneSet", "Description", "NumberGenes")
  gmtLines_description$NumberGenes <-
    as.numeric(gmtLines_description$NumberGenes)
