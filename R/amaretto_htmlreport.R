...
#'                     output_address='./')
#'}
AMARETTO_HTMLreport <- function(AMARETTOinit, AMARETTOresults, CNV_matrix = NULL, MET_matrix = NULL, SAMPLE_annotation = NULL, 
    ID = NULL, VarPercentage, hyper_geo_test_bool = FALSE,  hyper_geo_reference = NULL, output_address = "./", MSIGDB = FALSE, GMTURL = FALSE) {
  ...
  NrModules <- AMARETTOresults$NrModules
    ...
    ModuleOverviewTable <- foreach(ModuleNr = 1:NrModules, .packages = c("AMARETTO", "tidyverse", "DT", "rmarkdown")) %dopar% {
        ...
        dt_regulators <- datatable(rownames_to_column(as.data.frame(ModuleRegulators),"RegulatorIDs") %>% 
                                      dplyr::rename(Weights = "ModuleRegulators") %>% 
                                      mutate(RegulatorIDs = paste0("<a href=\"https://www.genecards.org/cgi-bin/carddisp.pl?gene=", RegulatorIDs, "\">", RegulatorIDs, "</a>")), 
                                   class = "display", 
                                   extensions = "Buttons", 
                                   rownames = FALSE, 
                          ...
                          formatRound("Weights", 3) %>% 
                          formatStyle("Weights", color = styleInterval(0, c("darkblue", "darkred")))
        
        if (hyper_geo_test_bool) {
            output_hgt_filter <- output_hgt %>% 
                                          dplyr::filter(Testset == paste0("Module_", as.character(ModuleNr))) %>% 
                                          dplyr::arrange(padj)
            output_hgt_filter <- left_join(output_hgt_filter,GeneSetDescriptions, by = c(Geneset = "GeneSet")) %>% 
                                          mutate(overlap_perc = n_Overlapping/NumberGenes) %>% 
                                          dplyr::select(Geneset, Description, n_Overlapping, Overlapping_genes, overlap_perc, p_value, padj)
            if (MSIGDB == TRUE & GMTURL == FALSE) {
                dt_genesets <- datatable(output_hgt_filter %>% 
                                            mutate(Geneset = paste0("<a href=\"http://software.broadinstitute.org/gsea/msigdb/cards/", Geneset, "\">", gsub("_", " ", Geneset), "</a>")), 
                                        class = "display",
                                        extensions = "Buttons",
                                        rownames = FALSE, 
                                        options = list(pageLength = 10, dom = "Bfrtip", buttons = c("csv", "excel", "pdf")), 
                                        colnames = c("Gene Set Name", "Description", "# Genes in Overlap", "Overlapping Genes", "Percent of GeneSet overlapping", "p-value", "FDR q-value"), 
                                        escape = FALSE) %>% 
                ...
                dt_genesets <- datatable(output_hgt_filter %>% 
                                              mutate(Geneset = paste0("<a href=\"", Description, "\">", gsub("_", " ", Geneset), "</a>")) %>% 
                                              dplyr::select(-Description), 
                                        class = "display", 
                                        extensions = "Buttons", 
                                        rownames = FALSE, 
    ...
    dt_overview <- datatable(ModuleOverviewTable %>% 
        mutate(ModuleNr = paste0("<a href=\"./modules/module", ModuleNr, ".html\">Module ", ModuleNr, "</a>")), 
        class = "display", 
        extensions = "Buttons", 
        rownames = FALSE, 
        options = list(pageLength = 10, dom = "Bfrtip", buttons = c("csv", "excel", "pdf")), 
    ...
    all_targets <- rownames_to_column(data.frame(AMARETTOinit$ModuleMembership),"Genes") %>% 
                      dplyr::rename(Module = "AMARETTOinit.ModuleMembership") %>% 
                      mutate(Type = "Target")
    all_regulators <- melt(rownames_to_column(as.data.frame(AMARETTOresults$RegulatoryPrograms),"Module"), id.vars = "Module") %>% 
                        dplyr::filter(value > 0) %>% 
                        dplyr::select(variable, Module) %>% 
                        mutate(Module = sub("Module_", "", Module), Type = "Regulator") %>% 
    ...
    if (hyper_geo_test_bool) {
        dt_genesetsall <- datatable(left_join(output_hgt %>% 
                                          dplyr::filter(padj < 0.05 & n_Overlapping > 1) %>%
                                          dplyr::group_by(Geneset) %>% 
                                          dplyr::mutate(Testset = paste0("<a href=\"./modules/module", sub("Module_", "", Testset), ".html\">", Testset, "</a>")) %>% 
                                          summarise(Modules = paste(Testset, collapse = ", ")), GeneSetDescriptions, by = c(Geneset = "GeneSet")) %>% 
                                      mutate(Geneset = gsub("_", " ", Geneset), Modules = gsub("_", " ", Modules)),
                                  class = "display", 
                                  extensions = "Buttons", 
                                  rownames = FALSE, 
