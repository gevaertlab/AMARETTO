...
AMARETTO_Download <-
  function(CancerSite,
    ...
    MAEO <-
      curatedTCGAData::curatedTCGAData(CancerSite, assays, FALSE)
    saveRDS(MAEO,
            file = paste0(TargetDirectory, CancerSite, "_RNASeq_MAEO.rds"))
    dataType = "analyses"
...
get_firehoseData <-
  function(downloadData = TRUE,
    ...
    if (TCGA_acronym_uppercase %in% cancers_acronyms) {
      ...
      urlData <-
        strsplit2(urlData, paste(dataType, "__", sep = ""))
      urlData <- urlData[, 2:dim(urlData)[2]]
      urlData <- strsplit2(urlData, "/")
      urlData <- urlData[, 1]
      urlData <- as.POSIXct(strptime(urlData, "%Y_%m_%d"))
      dateData <-
        as.Date(as.character(urlData[which(!is.na(urlData))]))
      lastDate <-
        dateData[match(summary(dateData)[which(names(summary(dateData)) == "Max.")], dateData)]
      ...
      while (length(grep("was not found", urlData)) > 0) {
        ...
        dateData <-
          dateData[-which(dateData == (summary(dateData)[which(names(summary(dateData)) == "Max.")]))]
        lastDate <-
          dateData[match(summary(dateData)[which(names(summary(dateData)) == "Max.")], dateData)]
        lastDate <- gsub("-", "_", as.character(lastDate))
        lastDateCompress <- gsub("_", "", lastDate)
      ...
      print(fileName)
      saveDir <-
        paste(saveDir, "gdac_", lastDateCompress, "/", sep = "")
      if (downloadData) {
        cat("\tDownloading",
            dataFileTag,
            "data, version:",
        ...
        if (fileType == "tar.gz" && untarUngzip) {
          ...
          untar(tarfile)
          fileToRemove <-
            strsplit2(gdacURL, "/")[, ncol(strsplit2(gdacURL, "/"))]
        ...
        finalDir <-
          substr(finalDir, start = 0, stop = (nchar(finalDir) - 1))
      ...
      return(cat(
        paste0("No data correspond to cancer ", TCGA_acronym_uppercase, "\n")
      ))
    }
  }
...
AMARETTO_ExportResults <-
  function(AMARETTOinit,
    ...
    output_dir <-
      paste0("AMARETTOresults_", gsub("-|:", "", gsub(" ", "_", Sys.time())))
    ...
    if (Heatmaps == TRUE) {
      foreach(ModuleNr = 1:NrModules,
              .packages = c("AMARETTO")) %dopar% {
                pdf(file = file.path(
                  data_address,
                  output_dir,
                  paste0("Module_", as.character(ModuleNr), ".pdf")
                ))
    ...
    save(AMARETTOresults,
         file = file.path(data_address, output_dir, "/amarettoResults.RData"))
    save(AMARETTOinit,
         file = file.path(data_address, output_dir, "/amarettoInit.RData"))
    
    # save some tables that might be useful for further analysis
    write_gct(
      AMARETTOresults$ModuleData,
      file.path(data_address, output_dir, "/ModuleData_amaretto.gct")
    )
    write_gct(
      AMARETTOresults$ModuleMembership,
      file.path(data_address, output_dir, "/ModuleMembership_amaretto.gct")
    )
    write_gct(
      AMARETTOresults$RegulatoryProgramData,
      file.path(
        data_address,
        output_dir,
        "/RegulatoryProgramData_amaretto.gct"
      )
    )
    write_gct(
      ...
      file.path(
        data_address,
        output_dir,
        "/RegulatoryPrograms_amaretto.gct"
      )
    )
    readr::write_tsv(
      as.data.frame(AMARETTOresults$AllGenes),
      file.path(data_address, output_dir, "/AllGenes_amaretto.tsv")
    )
    readr::write_tsv(
      as.data.frame(AMARETTOresults$AllRegulators),
      file.path(data_address, output_dir, "/AllRegulators_amaretto.tsv")
    )
    readr::write_tsv(
      as.data.frame(AMARETTOresults$NrModules),
      file.path(data_address, output_dir, "/NrModules_amaretto.tsv")
    )
    
    # zip the file
    zip(
      zipfile = file.path(data_address, output_dir),
...
write_gct <- function(data_in, file_address) {
  ...
  data_in <-
    rownames_to_column(as.data.frame(data_in), "Name") %>% dplyr::mutate(Description = Name) %>% dplyr::select(Name,
                                                                                                               Description, everything())
  write(header_gct, file = file_address, append = FALSE)
  readr::write_tsv(data_in,
                   file_address,
                   append = TRUE,
                   col_names = TRUE)
}
