...
AMARETTO_Preprocess <- function(CancerSite, DataSetDirectories) {
  ...
  MinPerBatch = 5
  MAEO <-
    readRDS(paste0(DataSetDirectories[1], "/", CancerSite, "_RNASeq_MAEO.rds"))
  ...
  if (length(intersect(CancerSite, AllCancers)) > 0) {
    ...
    eval(parse(text = paste(
      "MET_TCGA=MethylStates$", CancerSite, sep = ""
    )))
  ...
  cat(
    "\tFound",
    length(rownames(MA_TCGA)),
    "genes and",
    length(colnames(MA_TCGA)),
    "samples for MA data.\n"
  )
  cat(
    "\tFound",
    length(rownames(CNV_TCGA)),
    "genes and",
    length(colnames(CNV_TCGA)),
    "samples for GISTIC data before overlapping.\n"
  )
  cat(
    "\tFound",
    length(rownames(MET_TCGA)),
    "genes and",
    length(colnames(MET_TCGA)),
    "samples for MethylMix data before overlapping.\n"
  )
  ...
  cat(
    ...
    length(OverlapSamples),
    "samples for GISTIC data.\n"
  )
...
TCGA_Load_GISTICdata <- function(GisticDirectory) {
  ...
  return(
    list(
      CGH_Data_Segmented = CGH_Data,
      CGH_Data_Thresholded = CGH_Data_Thresholded,
      AMPgenes = AMPgenes,
      DELgenes = DELgenes
    )
  )
...
TCGA_Load_MolecularData <- function(MAEO_ge) {
  ...
  NrMissingsPerGene = apply(MET_Data, 1, function(x)
    sum(is.na(x))) / ncol(MET_Data)
  cat(
    "Removing",
    sum(NrMissingsPerGene > MissingValueThreshold),
  ...
  NrMissingsPerSample = apply(MET_Data, 2, function(x)
    sum(is.na(x))) / nrow(MET_Data)
  cat(
    "Removing",
    sum(NrMissingsPerSample > MissingValueThreshold),
    "patients with more than 10% missing values.\n"
  )
  if (sum(NrMissingsPerSample > MissingValueThreshold) > 0)
    MET_Data = MET_Data[, NrMissingsPerSample < MissingValueThreshold]
...
#' @examples
TCGA_BatchCorrection_MolecularData <-
  function(GEN_Data, BatchData, MinInBatch) {
    ...
    if (sum(PresentSamples) != length(colnames(GEN_Data)))
      BatchDataSelected = BatchData[-which(PresentSamples == FALSE),]
    ...
    if (length(colnames(GEN_Data)) - length(which(colnames(GEN_Data) %in% SamplesToBeRemoved)) > 5) {
      # just checking if we have enough samples after removing the too small batches
      if (length(which(colnames(GEN_Data) %in% SamplesToBeRemoved)) > 0) {
        cat("Removing",
            length(which(
              colnames(GEN_Data) %in% SamplesToBeRemoved
            )),
...
TCGA_GENERIC_BatchCorrection <- function(GEN_Data, BatchData) {
  ...
  if (sum(PresentSamples) != length(colnames(GEN_Data)))
    BatchDataSelected = BatchData[-which(PresentSamples == FALSE),]
  BatchDataSelected$Batch <- factor(BatchDataSelected$Batch)
  BatchDataSelected$ArrayName <-
    factor(BatchDataSelected$ArrayName)
...
ComBat_NoFiles <-
  function(dat,
    ...
    if (filter) {
      ...
      cat(
        "Filtered genes absent in more than",
        filter,
        "of samples. Genes remaining:",
        nrow(dat),
        "; Genes filtered:",
        ngenes - nrow(dat),
        "\n"
    ...
    if (!NAs) {
      B.hat <-
        solve(t(design) %*% design) %*% t(design) %*% t(as.matrix(dat))
    } else {
    ...
    if (!NAs) {
      var.pooled <-
        ((dat - t(design %*% B.hat)) ^ 2) %*% rep(1 / n.array, n.array)
    } else {
    ...
    s.data <-
      (dat - stand.mean) / (sqrt(var.pooled) %*% t(rep(1, n.array)))
    cat("Fitting L/S model and finding priors\n")
    batch.design <- design[, 1:n.batch]
    if (!NAs) {
      gamma.hat <-
        solve(t(batch.design) %*% batch.design) %*% t(batch.design) %*% t(as.matrix(s.data))
    } else {
    ...
    for (i in batches) {
      delta.hat <-
        rbind(delta.hat, apply(s.data[, i], 1, var, na.rm = TRUE))
    }
    ...
    if (par.prior) {
      ...
      for (i in 1:n.batch) {
        temp <-
          int.eprior(as.matrix(s.data[, batches[[i]]]), gamma.hat[i,], delta.hat[i,])
        gamma.star <- rbind(gamma.star, temp[1,])
        delta.star <- rbind(delta.star, temp[2,])
      }
    }
    ...
    for (i in batches) {
      bayesdata[, i] <-
        (bayesdata[, i] - t(batch.design[i,] %*% gamma.star)) / (sqrt(delta.star[j,]) %*% t(rep(1,
                                                                                                n.batches[j])))
      j <- j + 1
    }
    bayesdata <-
      (bayesdata * (sqrt(var.pooled) %*% t(rep(1, n.array)))) + stand.mean
    if (write) {
...
filter.absent <- function(x, pct) {
  ...
  col <- length(x) / 2
  pct.absent <-
    (sum(x[2 * (1:col)] == "A") + sum(x[2 * (1:col)] == "M")) / col
  if (pct.absent > pct) {
    present <- FALSE
  }
  present
...
list.batch <- function(saminfo) {
  ...
  for (i in 1:nlevels(tmp1)) {
    batches <-
      append(batches, list((1:length(tmp1))[tmp1 == levels(tmp1)[i]]))
  }
  batches
}
...
#' @examples
TCGA_GENERIC_MergeData <-
  function(NewIDListUnique, DataMatrix, MergeMethod) {
